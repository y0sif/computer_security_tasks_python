...
class DES:
    ...
    def hex_to_binary(hex_str):
        ...
        # Convert to binary string and ensure it is 64 bits long
        binary_string = bin(int_value)[2:]  # Remove '0b' prefix
    ...
    def left_shift(half, shifts):
        shifts = shifts % len(half)  # Safe shift
        return half[shifts:] + half[:shifts]
    
    def encrypt(self, plain_text_hex, key_hex):
        plain_bin = self.hex_to_binary(plain_text_hex)
        key_bin = self.hex_to_binary(key_hex)
        
        # Step 1: Initial Permutation
        permuted = self.permute(plain_bin, self.IP)
        ...
        subkeys = self.generate_sub_keys(key_bin)
        
        # Step 4: 16 Rounds
        for i in range(16):
            temp = R
            R = self.xor(L, self.feistel(R, subkeys[i]))
            L = temp
        
        # Step 5: Final Permutation
        combined = R + L
        cipher_bin = self.permute(combined, self.FP)
        return self.binary_to_hex(cipher_bin)
    
    def decrypt(self, cipher_text_hex, key_hex):
        cipher_bin = self.hex_to_binary(cipher_text_hex)
        key_bin = self.hex_to_binary(key_hex)
        
        permuted = self.permute(cipher_bin, self.IP)
        L = permuted[:32]
        R = permuted[32:]
        ...
        combined = R + L
        plain_bin = self.permute(combined, self.FP)
        return self.binary_to_hex(plain_bin)
